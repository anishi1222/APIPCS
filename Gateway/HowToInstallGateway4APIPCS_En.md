...
- A virtual or phisical node where gateway software appliance is installed has been already ready.
...
3. Configuration for host name resolution.
    - Configure /etc/hosts, /etc/sysconfig/network, or /etc/hostname
4. If needed, create Swapfile (In case of OCI-C, Swapfile is required. In case of OCI, swapfile is already configured.)
    ```bash
    # Create 4GB Swapfile
...
4. Reflect configured environment variables to current session.
    ...
    . ~oracle/.bash_profile
    ```
5. Modify random source in JDK（If installing JDK as 'root', configuration is required as 'root'.）
...
2. Install, create WebLogic Domain, and start gateway
    ```bash
    # Without action parameters, you can install and configure gateway. This is the same action as 'install-configure'.
...
1. Add created gateway node to logical gateway specified in gateway-props.json
    ```bash
