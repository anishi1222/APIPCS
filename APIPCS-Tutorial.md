...
<td>ゲートウェイ管理者<br />
（ゲートウェイ管理者）</td>
<td>ゲートウェイとデプロイ済のAPIを作成、管理する（通常ハードウェア管理者もしくはDevOps管理者）。<br>ゲートウェイのデプロイメント権限を管理する。分析データを要求し、ゲートウェイの性能を確認する。</td><td>API-ゲートウェイ-user</td>
</tr>
<tr>
...
この章では、Production ゲートウェイへのデプロイ権限を持つAPI管理者の役回りで演習を進めます。デプロイ・リクエストは自動的に承認されるので、ゲートウェイ管理者に切り替えてデプロイ・リクエストを承認する必要はありません。すでにAPIをデプロイ済なので、APIをゲートウェイに再デプロイすると、先ほど作成したポリシーが有効になります。
